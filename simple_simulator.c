	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case ADD:
		    // Verifica se deve somar o Carry anterior (ADDC)
                    if(carry == 1)
                        temp_calc = (long long)x + y + FR[CARRY];
                    else
                        temp_calc = (long long)x + y;

                    // Verifica se estourou 16 bits (Carry Out)
                    if(temp_calc > MAX_VAL) {
                        auxFRbits[CARRY] = 1;
                        result = temp_calc & MAX_VAL; // Mantém apenas os 16 bits inferiores
                    } else {
                        auxFRbits[CARRY] = 0;
                        result = (unsigned int)temp_calc;
                    }
                    break;
				case SUB:
		    // Verifica se deve subtrair o Carry anterior (SUBC / Borrow)
                    if(carry == 1)
                        temp_calc = (long long)x - y - FR[CARRY];
                    else
                        temp_calc = (long long)x - y;

                    // Verifica se deu negativo
                    if(temp_calc < 0) {
                        auxFRbits[NEGATIVE] = 1;
                        result = temp_calc & MAX_VAL; // Transforma em complemento de 2 para 16 bits
                    } else {
                    ...
                    break;
				case MULT:
		    // Verifica se deve somar o Carry ao produto (raro, mas previsto na spec)
                    if(carry == 1)
                        temp_calc = (long long)x * y + FR[CARRY];
                    else
                        temp_calc = (long long)x * y;

                    // Verifica Overflow Aritmético (resultado > 65535)
                    if(temp_calc > MAX_VAL) {
                        auxFRbits[ARITHMETIC_OVERFLOW] = 1;
                        result = temp_calc & MAX_VAL;
                    } else {
		    ...
		    // Verifica divisão por zero
                    if(y == 0) {
                        result = 0; // Resultado indefinido, definimos como 0
